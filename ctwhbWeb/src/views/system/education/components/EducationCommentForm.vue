<template>
  ...
  <el-row justify="center" class="mt-3">
    <el-button @click="handleAdd" round>+ 添加教育评论</el-button>
  </el-row>
</template>
<script setup lang="ts">
import { EducationApi } from '@/api/system/education'

const props = defineProps<{
  educationId: undefined // 教育编号（主表的关联字段）
}>()
...
watch(
  ...
  async (val) => {
    ...
    try {
      ...
      formData.value = await EducationApi.getEducationCommentListByEducationId(val)
    } finally {
...
const handleAdd = () => {
  ...
  row.educationId = props.educationId
